...
export function TabsList(props: {
  ...
  return (
    <div
      {...props}
      className={cn(
        "flex h-auto max-sm:w-full flex-col items-center justify-start rounded-md bg-muted p-1 text-muted-foreground",
        props.className,
      )}
    />
  );
...
export const TabsTrigger = (props: {
  ...
  return (
    <Link
      {...props}
      className={cn(
        "inline-flex w-full items-center max-sm:justify-center justify-start whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm",
        props.className,
      )}
